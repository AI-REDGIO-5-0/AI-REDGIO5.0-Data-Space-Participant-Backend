...
import type { ProviderModuleOptions } from './provider-module-options.interface';
...
export class ProviderService {
  ...
  async updateDataset(id: string, file: any, token: string) {
    ...
    metadata.filesize = file.data.size
      ? Number((file.data.size / (1024 * 1024)).toFixed(4))
      : metadata.filesize;
  ...
  private async updatePublishedMetadata(
    ...
    const datasetURL = await this.minioService.getFileUrl(
      metadata.title,
      this.options.bucketName,
    );
    try {
  ...
  async publish(id: any, data: any, token: string) {
    const metadata = await this.metadataService.findMetadata(
  ...
  async findTransactions() {
    ...
    const transactions = await firstValueFrom(
      this.httpService
        .get(
          `${this.options.cloudCatalogUrl}api/cloud-catalog/transaction/${this.options.userId}`,
          {
            headers: {
              Accept: 'application/json',
              'content-type': 'application/json',
    ...
    for (let i = 0; i < transactions.length; i++) {
      if (transactions[i].buyer !== this.options.userId) {
        const existing = await transactionRepo.findOne(Transactions, {
