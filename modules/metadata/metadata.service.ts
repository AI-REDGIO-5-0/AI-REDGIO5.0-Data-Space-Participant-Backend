...
export class MetadataService {
  ...
  constructor(
    ...
    private readonly repo: EntityRepository<Metadata>,
    @InjectRepository(AccessPolicy)
    private readonly policiesRepo: EntityRepository<AccessPolicy>,
  ) {}
    ...
    const metadata = {
      ...
      keywords:
        data.keywords.length > 0
          ? data.keywords
          : data.keywords.split(',').map((kw: any) => kw.trim().toLowerCase()),
