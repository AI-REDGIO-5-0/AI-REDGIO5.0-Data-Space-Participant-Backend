...
export class MinioService {
  ...
  async updateFile(
    ...
    try {
      ...
      const prevStat = await this.minioClient.statObject(
        bucketName,
        latestFile.name,
      );
  ...
  async retrieveFile(data: any, fileExt: string): Promise<void> {
    try {
      ...
      // if (!bucketExists) {
      //   await this.minioClient.makeBucket(data.bucket);
      // }
      ...
      console.log(
        `File '${data.fileName}' downloaded from URL and uploaded to 'test1'`,
      );
    } catch (error) {
  ...
  async getCorrectUrl(bucketName: string, fileName: string, extension: string) {
    const fullFileName = extension ? `${fileName}.${extension}` : fileName;
    const contentDisposition = `attachment; filename="${fullFileName}"`;
  ...
  async downloadFile(objectName: string, bucketname: string) {
    console.log(`Downloading file: ${objectName} from bucket: ${bucketname}`);
    let metaData;
    let stream;
    try {
      // 1. Get file metadata (required for content type and file name)
      metaData = await this.minioClient.statObject(bucketname, objectName);
