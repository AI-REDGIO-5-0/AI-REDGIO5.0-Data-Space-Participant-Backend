...
export class ConsumerService {
  ...
  async getFile(fileName: string, query: any) {
    const url = await this.minioService.getCorrectUrl(this.options.bucketName, fileName, query.ext);
    return url;
  }

  async getFileMinioMetadata(assetId: string) {
    const asset = await this.metadataService.findByAssetId(assetId);
    return asset.fileExt
  }

  async downlaodFile(assetId: string) {
    const asset = await this.metadataService.findByAssetId(assetId);
    return {title: asset.title, ext: asset.fileExt};
  }
}
